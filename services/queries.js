...
const checkBalance = async (accountId, tokenId, client) => {
	let balanceCheckTx = [];
	try {
		balanceCheckTx = await new AccountBalanceQuery().setAccountId(accountId).execute(client);
		console.log(
		...
		balanceCheckTx = await new AccountBalanceQuery().setContractId(accountId).execute(client);
		console.log(
